...
data_crossvalidation=pd.read_csv("/home/karthik/Desktop/mini/model1/altered_dataset2/cross.csv",header=None,skiprows=1)
df_crossvalidation=DataFrame(data_crossvalidation)
ycross=df_crossvalidation.iloc[:, 3:4]
xcross=df_crossvalidation.iloc[:, 0:3]


output_amt=[]
output_test=[]
output_crossvalidation=[]
...
for i in range(no_neurons):
	...
	#w_wasted[i]=gradient_descent(no_examples,x,y_waste,np.transpose(w_wasted[i]),no_iter,0.000001,b[i])

for j in range(0,no_neurons):
	output_amt.append(np.transpose(eqn(x,np.transpose(w[j]),b[j])))
	#output_wasted.append(np.transpose(eqn(x,np.transpose(w_wasted[j]),b[j])))
	output_test.append(np.transpose(eqn(xtest,np.transpose(w[j]),b[j])))
...
#print("rmse of 5 neurons for training data:\n\n")

#for i in range(0,no_neurons):
#	print(np.sqrt(metrics.mean_squared_error(y_amt,np.transpose(output_amt[i]))))


#print("rmse of 5 neurons for testing data:\n\n")
#for i in range(0,no_neurons):
#	print(np.sqrt(metrics.mean_squared_error(ytest,np.transpose(output_test[i]))))
	#print(np.sqrt(metrics.mean_squared_error(y_waste,np.transpose(output_wasted[i]))))


#print("rmse of 5 neurons for cross validation data:\n\n")
#for i in range(0,no_neurons):
#	print(np.sqrt(metrics.mean_squared_error(ycross,np.transpose(output_crossvalidation[i]))))
...
print(output_crossvalidation)

#print(np.array(y_amt))
#print("\n\n")
#print(output_amt)
#print("\n\n")
#print(output_wasted[0])